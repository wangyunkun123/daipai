...
def load_series_rhythm():
    """加载组图节奏知识，注入 PLANS_PROMPT（🥇 丰富场景时启用）。

    优先从知识库 Markdown 文件加载，文件不存在时使用内置精简版兜底。
    返回精简但完整的组图规则文本——四条节奏线 + 编排流程 + 检查清单 + 常见错误。
    """
    # 尝试加载完整知识文件
    sr_path = os.path.join(_KNOWLEDGE_DIR, "series-rhythm", "_index.md")
    ...
    # ── 内置精简版兜底（部署环境无知识文件时使用）──
    return """## 组图的四条节奏线

### 节奏线 1：色彩统一线
- 9 张照片必须共享同一色温方向（全暖调/全冷调/全中性）
- 9 张共享一个主色调（如蓝绿基调/暖黄基调/青橙分离）
- 9 张共享同一饱和度级别（饱和统一 = 情绪强度统一）
- 选 1 个强调色，在 2-3 张照片中复现——跨照片的色彩对话，组图的「韵脚」
- ⛔ 不能第 3 张全暖 + 第 4 张全冷 → 色彩割裂
- 色彩统一 ≠ 每张一样：日系蓝绿基调下，第 3 张偏蓝（海）、第 5 张偏绿（草地）、第 8 张偏暖（黄昏）都可以

### 节奏线 2：景别张弛线
- 9 张必须覆盖 ≥ 4 种景别类型
- 相邻两张景别差异保持 1 级（如中景→近景），偶尔 2 级制造节奏变化
- 景别分配参考（弹性框架）：
  1（开场）远景——定调 · 2-3 中景——推进+展开 · 4 近景——亲密
  5（高点）创意/近景——惊喜 · 6 中景——回落 · 7 特写——凝视
  8 中景/远景——回收 · 9（收尾）远景/创意——余韵
- ⛔ 9 张全中景 → 观众分不清哪张是哪张
- ⛔ 远景直接跳特写 → 仅在有叙事目的时使用（如突然的亲密凝视），不作为默认过渡

### 节奏线 3：情绪曲线（参照电影三幕结构）
- 1-3 张（第一幕·起）：定调情绪（中低）——建立世界的情绪基调
- 4-7 张（第二幕·承/转）：情绪上升 + 高点——观众注意力峰值
- 8-9 张（第三幕·合）：情绪回落 + 余韵——让观众带着余韵离开
- 按风格定制曲线形状：
  · 日系清新：整体平缓温和，高点温柔不爆发
  · 电影感：明显起伏——低起→中段爬升→高点→回落→余韵
  · 纪实粗粝：全程紧张，无明显起伏
- ⛔ 不能 9 张都是微笑——至少设计一个情绪高点和一个安静时刻

### 节奏线 4：叙事弧线
- 1 开场：环境/世界的样貌 · 2-3 引入：人物出现 · 4-6 发展：正在发生什么
- 7 转折：一个意外——不同于前面 6 张的构图/景别/情绪
- 8-9 收束：故事结束了，但余韵还在

## 组图节奏的设计流程

生成方案后逐条检查这 7 步：
1. 确定整组审美基调（风格 → 色彩基调 → 情绪基调）
2. 为每张分配景别岗位（开场远景/中景主力/近景亲密/特写细节/创意惊喜）
3. 绘制情绪曲线（起承转合的位置和强度）
4. 检查景别跳跃（相邻差异是否合理）
5. 编排色彩呼应（强调色在 2-3 张中复现）
6. 设计首尾呼应（第 1 张和第 9 张的关系——景别/色彩/构图/叙事四选一）
7. 定位破格方案（放第 5 或第 7 张——中段高点或转折点，不放开头或结尾）

## 首尾呼应

第 1 张和第 9 张必须有至少一种呼应关系（序列位置效应——开头和结尾记忆最深）：
- 景别呼应：开场远景 → 收尾远景「从哪里来，回哪里去」
- 色彩呼应：第 1 张和第 9 张使用相同的强调色
- 构图呼应：相同的构图方式（如对称海天→对称日落）
- 叙事呼应：同一场景的不同状态（空椅子等待→空椅子离开）

## 组图节奏检查清单

生成后逐条自查：
☐ 色彩统一下限：9 张中 ≥ 7 张共享同一色彩基调？
☐ 景别覆盖：覆盖 ≥ 4 种景别类型？
☐ 景别跳跃：不存在无叙事理由的跳崖式跳跃？
☐ 情绪曲线：情绪有起伏？高点在哪一张？有叙事理由？
☐ 首尾呼应：第 1 张和第 9 张有至少一种呼应关系？
☐ 破格定位：破格方案放在有意义的中间位置（不放开头或结尾）？
☐ 单张独立性：每张单独看仍然是一张好照片？
...
| 错误 | 修正 |
|------|------|
| 9 张全中景——看完记不住任何一张 | 强制分配景别岗位 |
| 情绪全平——9 张都是微笑，没有记忆点 | 设计情绪曲线：至少一张深情/大笑/安静/回眸 |
| 没有首尾呼应——感觉「没完」 | 第 1 张和第 9 张添加呼应 |
| 色彩不统一——第 3 张全暖+第 4 张全冷 | 统一色温/色调方向 |
| 破格放开头或结尾——观众误以为整组都是破格风格 | 破格放第 5 或第 7 张 |
| 9 张同一张脸/同一个角度——换后期不换前期 | 每套的 shooter/subject 必须有实质变化 |"""
...
📎 技法锚点：极简高级 + 新中式""",
    ...
    "奶油风": """**奶油风 · 摄影可执行参数**
🎯 前期拍法（这些是拍摄现场能操作的——不是后期滤镜）：
- 光线核心：柔光是必须——找纱帘柔化的窗边光、阴天散射光。绝对不能有硬阴影！奶油风的"温柔感"来自全画面的柔和过渡。如果现场只有硬光，找半透明物体（纱帘/磨砂玻璃/白布）挡在光源和主体之间柔化
- 色彩控制：整个画面统一在一个暖色系里——不能出现冷色或高饱和色。背景/环境选暖色调材质（原木/米色墙面/亚麻布），避开蓝色/荧光色/高饱和物体
- 构图：中景为主——咖啡桌边/沙发/窗边。留白20-30%——奶白色墙面/桌面就是最好的留白载体。前景虚化（咖啡杯/花/半透明纱帘）增加层次
- 穿搭引导：米白/浅杏/燕麦色系——针织/棉麻材质。纯白色在强光下容易过曝发灰→选米白/本白/奶白色。不能穿黑色/深蓝/高饱和色——会破坏整个画面的温柔感
- 场景选择：暖色调咖啡厅/简约室内/窗边——背景不能杂乱。画面中所有物体颜色都应该是"暖色系+低饱和"
❌ 禁止：硬阴影（阳光直射/闪光灯/顶灯硬光）、冷色物体入画（蓝色杯子/黑色手机/荧光色衣物）、高饱和色块、杂乱背景、强反光面
📎 技法锚点：日系清新（米白奶咖+柔和到无阴影+高明度低对比）
📱 后期参考（仅 quick_edit，不混入前期字段）：iPhone鲜明度+80以上 → 肤色和暖色"发光"。HSL橙色明亮度拉高。色温+10~+15。对比度-8、高光-44""",
}
