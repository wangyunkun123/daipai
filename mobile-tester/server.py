...
### Step 4: 方向卡片 + 方案
按 zhichu 格式输出。每个方向自带 1-9 套拍摄方案：

🟢 现在就拍：可执行性最高的方向（必有）
🔥 最出片：辨识度最高的方向（有则给，没有就设 null）
✨ 脑洞大开：最酷的方向（有则给，没有就设 null——没东西不强凑）
...
每套方案：
- name: 方案名（能让人记住的，不是"方案1""方案2"）
- action: 具体操作步骤（站哪里、用什么焦段、怎么构图、怎么用光）
...
{{
  ...
  "directions": [
    {{
      ...
      "source_note": "来源标注",
      "plans": [
        {{"name": "方案名", "action": "具体操作步骤", "perspective": "换个思路"}}
      ]
    }},
    {{
      "id": "best",
...
如果一个方向没有合适的内容（如🔥没有高辨识度方向），把该方向的所有字段设为 null。
plans 按需给 1-9 套，不凑数。
