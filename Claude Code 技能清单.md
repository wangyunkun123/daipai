...
| 项目 | 详情 |
|------|------|
| **包名** | `ceeon/best-minds@best-minds` |
| **安装量** | 858+ |
| **安全评级** | 🟢 Gen Safe / Socket 0 alerts / Snyk Low Risk |

**用途**: 模拟器思维 — 不问"你怎么看"，而是问"这个问题世界上谁最懂？TA 会怎么说？"含 72 个领域专家 agent，基于 Karpathy 的 "LLMs as simulators" 理念。

**核心原则**: 找真正最懂的、模拟器思维（LLM 是模拟器不是实体）、基于真实言论、引用原话

**调用方式**: 说 "最强大脑" / "best minds" / "谁最懂这个" / "顶级专家"
...
| 项目 | 详情 |
|------|------|
| **包名** | `mattpocock/skills@grill-me` |
| **安装量** | 503.2K+ |
| **安全评级** | 🟢 Gen Safe / Socket 0 alerts / Snyk Low Risk |

**用途**: 对你的计划或设计进行无情"烤问"（grilling），通过高强度质询暴露弱点，打磨更坚固的方案。由 Matt Pocock 创作。

**调用方式**: 说 "grill me" / "烤问我" / "来拷打这个方案"
...
| 项目 | 详情 |
|------|------|
| **包名** | `slavingia/skills` (plugin marketplace) |
| **作者** | Sahil Lavingia（Gumroad 创始人，《小而美》作者）|
| **GitHub Stars** | 9,000+ |

**用途**: 将《The Minimalist Entrepreneur》全书方法论转化为 10 个可执行技能，覆盖创业全流程：社区发现 → 验证想法 → MVP → 流程化 → 客户获取 → 定价 → 营销 → 可持续增长 → 价值观 → 极简审查。

**10 个命令**: `/find-community` `/validate-idea` `/mvp` `/processize` `/first-customers` `/pricing` `/marketing-plan` `/grow-sustainably` `/company-values` `/minimalist-review`

**核心理念**: 社区优先于产品，先卖再建，流程化先于产品化。

**安装方式**: `/plugin marketplace add slavingia/skills` → `/plugin install minimalist-entrepreneur`
